...
# ligand_receptor_data_directory = glue("{root_directory}/ligand_receptor_data_0.2.1")
# light_data_directory = glue("{root_directory}/splitted_light_data_0.2")
# metadata_annotated_file = "/vast/projects/RCP/human_cell_atlas/metadata_annotated_0.2.1.rds" # glue("{root_directory}/metadata.rds")
# cell_type_df = "metadata_cell_type.csv"
#
# light_file_paths = dir(light_data_directory, full.names = TRUE)
# sample_ = basename(light_file_paths) |> tools::file_path_sans_ext()
# ligand_receptor_file_paths = glue("{ligand_receptor_data_directory}/{sample_}.rds")
# file_for_annotation_workflow = glue("{root_directory}/cell_sample_cell_type_df_for_annotation_workflow.rds")
#
# metadata_annotated  = readRDS(metadata_annotated_file)
#
# metadata_annotated |> distinct(cell_, sample_, cell_type, cell_type_harmonised, confidence_class) |>
# 	saveRDS(file_for_annotation_workflow)
#
#
# metadata_annotated |>
#   distinct(sample_, file_id) |>
#   # filter(sample_ %in% samples_to_use) |>
#   mutate(
#     input_file_path = glue("{light_data_directory}/{sample_}") |> as.character(),
#     output_file_path = glue("{ligand_receptor_data_directory}/{sample_}.rds" |> as.character())
#   ) |>
#
...
# 	mutate(memory = case_when(
# 		sample_ %in% c(
# 			"828c2edf33b7d07549bc0e0a0fe06cb7"
# 		)~ 160000,
# 		TRUE ~ memory
# 	)) |>
#
...
#                              glue("{..1}:{..2} {metadata_annotated_file} {cell_type_df}\n{tab}Rscript ligand_receptor_count.R {..2} {file_for_annotation_workflow} {..1}")
#                            )
#   ))  |>
#   pull(commands) |>
#   unlist() |>
#   write_lines(glue("~/PostDoc/CuratedAtlasQueryR/dev/ligand_receptor_count.makeflow"))
...
sample_ = basename(input_path_preprocessing) |> tools::file_path_sans_ext()
...
data =
  ...
  left_join(
	...
	# only high quality
	filter(confidence_class %in% 1:3) |>
	tidySingleCellExperiment::add_count(cell_type_harmonised) |>
	tidySingleCellExperiment::filter(n>=10)
