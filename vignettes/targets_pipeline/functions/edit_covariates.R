edit_covariates = function(tbl){
    ...
    disease_data_grouped =
      ...
      left_join(
        (function() {
        disease_csv <- Sys.getenv("DISEASE_GROUPING_CSV", unset = "")
        if (!nzchar(disease_csv)) {
          root <- Sys.getenv("IMMUNE_HEALTHY_BODY_MAP_ROOT", unset = "")
          if (nzchar(root)) {
            disease_csv <- file.path(
              root, "vignettes", "data", "source_tables",
              "disease_data_grouped_further.csv"
            )
          }
        }
        if (!nzchar(disease_csv) || !file.exists(disease_csv)) {
          stop(
            "Missing disease_data_grouped_further.csv. Set DISEASE_GROUPING_CSV ",
            "or IMMUNE_HEALTHY_BODY_MAP_ROOT (expects vignettes/data/source_tables/).",
            call. = FALSE
          )
        }
        readr::read_csv(disease_csv)
      })() |>
